  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div class="flex-1 min-w-0">
            ...
            >
              <template v-if="isRegistered">
                ...
                <span v-if="device?.modem?.network_mode" class="opacity-70">· {{ [device?.modem?.network_duplex, device?.modem?.network_mode].filter(Boolean).join(' ') }}{{ isRoaming ? ' ·' : '' }}</span>
    ...
    <div class="ui-panel-muted p-4 relative min-w-0 overflow-hidden">
      ...
      <div class="text-sm space-y-1.5 text-gray-700 dark:text-gray-200">
        ...
        <FieldRow label="短信中心号码" :value="device?.modem?.smsc || '--'" monospace copyable />
        <FieldRow label="归属运营商" :value="simOperatorDisplay" copyable />
        <FieldRow label="当前注册网络" :value="registeredNetworkDisplay" copyable />
